(function() {
  ...
  _.reject = function(collection, test) {
    ...
    return _.filter(collection, helper);
  };
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    for (var i = 0; i < array.length; i++) {
      if (!(iterator(array[i]) in seenBefore)) {
        seenBefore[iterator(array[i])] = 'banana';
  ...
  //loop through it for in
  //add into target object
  //return target
    for (var i = 1; i < arguments.length; i++) {
      for (var key in arguments[i]) {
        if (!(key in destination) || (destination[key] !== arguments[i][key])) {
  ...
  _.memoize = function(func) {
    ...
    var argSeen = {};
    var memoFun = function() {
      if (JSON.stringify(arguments) in argSeen) {
        return argSeen[JSON.stringify(arguments)];
      } else {
        var result = func.apply(this, arguments);
        argSeen[JSON.stringify(arguments)] = result;
        return result;
      }
    };
    return memoFun;
  ...
  //setTimeOut
  _.delay = function(func, wait, ...moreArgs) {
    var helper = function() {
      func.apply(null, moreArgs);
    };
      
    return setTimeout(helper, wait);
  };
